...
void c_scf_method(cp2k_transport_parameters cp2k_transport_params, cp2k_csr_interop_type S, cp2k_csr_interop_type KS, cp2k_csr_interop_type * P)
{
    ...
    for (uint a=0;a<pairlist.size();a++) {
        ...
        maxpair=max(maxpair,pairlist[a].size());
    }
    ...
    if (do_double) {
        int size_tot=S.nrows_total;
        int mid=size_tot/2;
        int cndof=contactvec[1].ndof;
        for (int i=0;i<contactvec[1].bandwidth;i++) {
            for (int ii=0;ii<=i;ii++) {
                copy_to_cp2k_csr(*P,0.5,\
                        mid+(-i-2)*cndof,\
                        mid+(-1)*cndof,\
                        mid+(-i-1+ii)*cndof,\
                        mid+(0+ii)*cndof,\
                        contactvec[0].ndof,cndof,MPI_COMM_WORLD);
                add_to_cp2k_csr(*P,0.5,\
                        mid+0*cndof,\
                        mid+(i+1)*cndof,\
                        mid+(-i-1+ii)*cndof,\
