...
void c_scf_method(cp2k_transport_parameters cp2k_transport_params, cp2k_csr_interop_type S, cp2k_csr_interop_type KS, cp2k_csr_interop_type * P)
{
   ...
   contactvec[0].ndof=cp2k_transport_params.contacts_data[1];
   contactvec[0].start=0;
   contactvec[0].inj_sign=+1;
   contactvec[0].n_ele=cp2k_transport_params.contacts_data[5];
   contactvec[1].bandwidth=cp2k_transport_params.contacts_data[0];
   contactvec[1].ndof=cp2k_transport_params.contacts_data[1];
   contactvec[1].start=OverlapCut->size_tot-contactvec[1].ndof*contactvec[1].bandwidth;
   contactvec[1].inj_sign=-1;
   contactvec[1].n_ele=cp2k_transport_params.contacts_data[5];
   contactvec[2].bandwidth=cp2k_transport_params.contacts_data[0];
   contactvec[2].ndof=cp2k_transport_params.contacts_data[1];
   contactvec[2].start=OverlapCut->size_tot/2;
   contactvec[2].inj_sign=+1;
   contactvec[2].n_ele=cp2k_transport_params.contacts_data[5];

   transport_params->bandwidth = contactvec[0].bandwidth;
   transport_params->n_cells   = OverlapCut->size_tot/contactvec[0].ndof;

   switch (transport_params->method) {
      case 0:
         if (!rank) cout << "Writing Matrices" << endl;
         transport_params->cutblocksize=0; //ADAPT THIS VALUE
         write_matrix(OverlapCut,KohnShamCut,transport_params);
      case 1:
     ...
     case 2:
         ...
         if (semiselfconsistent(OverlapCut,KohnShamCut,transport_params)) throw SCF_Exception(__LINE__,__FILE__);
         break;
     case 3:
     default:
